...
(function (global, factory) {
    ...
    function initState(vm) {
        ...
        if (opts.data) { //初始化数据
            ...
            console.log("initData调用前", vm);

            initData(vm);

            console.log("initData调用后", vm);
        } else {
    ...
    function createPatchFunction(backend) {
        ...
        function isUnknownElement$$1(
            ...
            return (!inVPre && // 标记 标签是否还有 v-pre 指令，如果没有则是false
                !vnode.ns && !(
    ...
    function enter(
        ...
        var isAppear = !context._isMounted || //是否已经调用过Mounted 生命周期函数
    ...
    */
    ...
    function parseHTML(
        ...
        while (html) { //循环html
            ...
            if (!lastTag || //lastTag 不存在
                ...
                if (textEnd >= 0) {
                    ...
                    while (!endTag.test(rest) && //匹配开头必需是</ 后面可以忽略是任何字符串
        ...
        function parseStartTag() {
            ...
            if (start) {
                ...
                while (!(end = html.match(startTagClose)) //没有到 关闭标签 > 标签
                    &&
        ...
        parseHTML(
            template, //字符串模板
            {
                ...
                chars: function chars(text) {
                    ...
                    if (text) {
                        var res;
                        if (!inVPre && //标记 标签是否还有 v-pre 指令，如果没有则是false
                            text !== ' ' && //你是空节点
    ...
    function markStatic$1(node) {
        ...
        if (node.type === 1) {
            ...
            // 1。组件无法更改插槽节点
            // 2。静态插槽内容无法热加载
            if (!isPlatformReservedTag(node.tag) && //保留标签 判断是不是真的是 html 原有的标签 或者svg标签
